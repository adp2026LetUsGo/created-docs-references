# AHS Blueprint Supplement V3.1.1
## Resolution of Three Architectural Tensions
...
```
AHS.Cell.ColdChain → 1 PostgreSQL DB: ahs_coldchain
...
AHS.ControlTower.BFF  ← NEW project (Minimal API, AOT-safe)
    ├── /bff/coldchain/*   → HTTP → AHS.Cell.ColdChain.API (+ cache)
...
```csharp
// AHS.ControlTower.BFF/Hubs/ColdChainHub.cs
// SignalR hub — receives Service Bus events, pushes to connected CT clients
public class ColdChainHub(IHubContext<ColdChainHub> hub) : Hub
{
...
public class BffEventConsumer(ServiceBusClient sb, IHubContext<ColdChainHub> hub)
    : BackgroundService
{
...
| T2 | CT data aggregation | BFF pattern: SignalR push for critical widgets (<1s), HybridCache 30s-5min for analytical | New project: `AHS.ControlTower.BFF`, `ColdChainHub` |
